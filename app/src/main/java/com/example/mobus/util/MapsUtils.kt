...
class MapsUtils
{
    ...
    //create a constructor to markers

    //Função que devolve as marcações para colocar no mapa
    public fun addMarkers(googlemap: GoogleMap, nome: String, descricao: String, latitude: String, longitude: String): Marker?
    {

        val icone = BitmapDescriptorFactory.fromResource(R.drawable.amoung)

        val markerOptions = MarkerOptions()
        markerOptions.position(formatLatLog(latitude.toString(), longitude.toString()))
    ...
    {
        val map = googlemap
        val fusedLocationClient: FusedLocationProviderClient = LocationServices.getFusedLocationProviderClient(comp)
        ...
        {
            comp.requestPermissions(arrayOf(android.Manifest.permission.ACCESS_FINE_LOCATION), 1)
        }
    ...
    {
        googlemap.moveCamera(CameraUpdateFactory.newLatLngZoom(formatLatLog(latitude,longitude), 15f))
    }
